// app.js - COMPLETO E ATUALIZADO PARA LAYOUT HÍBRIDO
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ▼▼▼ FUNÇÃO ATUALIZADA PARA LAYOUT DINÂMICO ▼▼▼
    function renderMenu() {
        ...
        CONFIG.cardapio.forEach(category => {
            ...
            category.itens.forEach((item, index) => {
                ...
                if (hasImage) {
                    // --- Layout COM Imagem (estilo iFood) ---
                    itemsHtml += `
                        <div class="menu-item com-imagem">
                            <div class="item-details">
                                <h3>${item.nome}</h3>
                                <p>${item.descricao}</p>
                                <div class="item-price">
                                    <span class="currency">R$</span>${inteiro},${centavos}
                                </div>
                            </div>
                            <div class="item-action">
                                <img src="${item.imagem}" alt="${item.nome}" class="item-image">
                                <button class="add-to-cart-btn" data-item-id="${itemId}">Adicionar</button>
                            </div>
                        </div>
                    `;
                } else {
                    // --- Layout SEM Imagem (estilo original) ---
                    itemsHtml += `
                        <div class="menu-item sem-imagem">
                            <div class="item-details">
                                <h3>${item.nome}</h3>
                                <p>${item.descricao}</p>
                            </div>
    ...
    function renderFooterAndInfoModal() {
        ...
        document.getElementById('info-modal-body').innerHTML = `
            <div class="info-block"><i class="fas fa-map-marker-alt"></i> ${info.endereco}</div>
            <div class="info-block"><i class="fas fa-clock"></i> ${info.horario}</div>
        `;
    }
            ...
            cartItemsContainer.innerHTML += `
                <div class="cart-item-row">
                    <div class="cart-item-main">
                        <div class="cart-item-info"><h4>${item.nome}</h4><span>R$ ${item.preco.toFixed(2).replace('.', ',')}</span></div>
                        <div class="quantity-controls"><button onclick="changeQuantityInCart('${item.cartItemId}', -1)">-</button><span>${item.quantity}</span><button onclick="changeQuantityInCart('${item.cartItemId}', 1)">+</button></div>
                    </div>
                    ${addonsHtml}
                </div>`;
        });
    ...
    function finalizeOrder() {
        if (!isStoreOpen) { alert('A loja está fechada no momento e não pode finalizar o pedido.'); return; }
        if (cart.length === 0) { alert('Seu carrinho está vazio.'); return; }
        const clientName = document.getElementById('client-name').value.trim();
        if (!clientName) { alert('Por favor, preencha seu nome.'); return; }
        const isDelivery = deliveryOptionSelect.value === 'delivery';
        const address = document.getElementById('client-address').value.trim();
        if (isDelivery && !address) { alert('Por favor, preencha seu endereço para entrega.'); return; }
        if (!paymentMethodSelect.value) { alert('Escolha a forma de pagamento.'); return; }
        let orderSummary = `*Novo Pedido - ${CONFIG.nomeLoja}*\n\n*Cliente:* ${clientName}\n\n--- *ITENS* ---\n`;
        cart.forEach(item => {
            orderSummary += `*${item.quantity}x* ${item.nome}\n`;
            item.adicionais.forEach(addon => { orderSummary += `  *+ ${addon.quantity}x ${addon.nome}*\n`; });
        });
        let subtotal = cart.reduce((sum, item) => {
            const addonsTotal = item.adicionais.reduce((adSum, ad) => adSum + ad.preco * ad.quantity, 0);
            return sum + (item.preco * item.quantity) + addonsTotal;
        }, 0);
        const deliveryFee = isDelivery ? CONFIG.taxaEntrega : 0;
        const total = subtotal + deliveryFee;
        ...
        if (paymentMethodSelect.value === 'dinheiro') {
            ...
            if (changeFor) { orderSummary += `*Troco para:* R$ ${changeFor}\n`; }
        }
